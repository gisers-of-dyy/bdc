    ...
    while True:
        ...
        if (rowIndex % 5 == 0):
            PointAttribute = dict(fruitId= fruitIdTemp, fruitCategoryId = fruitCategoryIdTemp,
                              pointName= tempList[0], pointId= tempList[1], pointLeveld= tempList[2],
                              geoDatum = tempList[3],heigDatum =tempList[4],producDate = tempList[5])
...
#     newSheet = book.add_sheet(sheetname='GNSS点整理', cell_overwrite_ok=True)
...
#
#     column0 = ["fruitId", "x", "y", "点名", "fruitCategoryId", "点号", "等级", "大地基准", " 高程基准","生产时间"]
#     k = 0
...
def writeShp(outFileName):
    ...
    pointId = ogr.FieldDefn("pointId", ogr.OFTString)
    #oFieldName.SetWidth(100)
    oLayer.CreateField(pointId, 1)
    ...
    #   2.8 创建一个叫geoDatum的字符型属性
    heightDatum = ogr.FieldDefn("heightDatum", ogr.OFTString)
    oLayer.CreateField(heightDatum, 1)

    #   2.9 创建一个叫producDate的字符型属性
...
locationFileName = "D:\Python\ExtractPointInfo\GNSS点\GNSS位置.txt"
attributeFileName = "D:\Python\ExtractPointInfo\GNSS点\GNSS属性.txt"
outFileName = "D:\Python\ExtractPointInfo\GNSS点\GNSS点整理.xls"
...
# writer = csv.writer(outFileName, delimiter='|', quoting=csv.QUOTE_ALL)
# #合并两个字典

#writer(outFileName)
outFileName1 = "D:\Python\ExtractPointInfo\GNSS点\GNSSPoint.shp"
